...
_install_sh="$(mktemp)"
wget -O "${_install_sh}" -q https://raw.githubusercontent.com/reviewdog/reviewdog/fd59714416d6d9a1c0692d872e38e7f8448df4fc/install.sh
sh "${_install_sh}" -b /tmp "${REVIEWDOG_VERSION}"
rm -f "${_install_sh}"
...
# Sanitize newlines to prevent GITHUB_ENV injection.
_safe_paths="$(printf '%s' "${black_check_file_paths[*]}" | tr -d '\n\r')"
# shellcheck disable=SC2129
echo "BLACK_CHECK_FILE_PATHS<<EOF" >> "$GITHUB_ENV"
echo "${_safe_paths}" >> "$GITHUB_ENV"
echo "EOF" >> "$GITHUB_ENV"
