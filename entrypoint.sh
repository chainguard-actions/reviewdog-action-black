...
# Tokenize list-style inputs into arrays (quote-aware, handles flags with values).
black_args=()
if [ -n "${INPUT_BLACK_ARGS}" ]; then
  while IFS= read -r -d '' t; do black_args+=("$t"); done \
    < <(printf '%s' "${INPUT_BLACK_ARGS}" | xargs printf '%s\0')
fi
